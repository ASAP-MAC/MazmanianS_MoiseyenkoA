...
panelD_FB.Ratio.df <- MetaPhlAn_Fp7.tse %>%
  colData() %>%
  as.data.frame()

comparisons <- combn(levels(panelD_FB.Ratio.df$genotype_treatment), 2, simplify = FALSE)
...
piece3 <- ggarrange(
  ...
  panelD_FB.Ratio + theme(legend.position = "none"), nrow = 1, labels = c("d", "e", "f"))
...
#   panelE_Obs.gg + theme(legend.position = "none"), 
#   panelE_Shannon.gg + theme(legend.position = "none"),
#   panelD_FB.Ratio + theme(legend.position = "none"),
#    nrow = 1
...
panelD_FB.Ratio.df <- MetaPhlAn_bCL.tse %>%
  colData() %>%
  as.data.frame()
...
ggsave(plot = FB.Ratio_plot, filename = file.path(params$basic_outdir, params$tax_level, "Bact_Firm_ratio.png"), dpi = 600, width = 6, height = 6)
FB.Ratio_plot
```
