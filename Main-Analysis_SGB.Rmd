...
ggsave(plot = microbiome_panel_MainText, filename = file.path(params$basic_outdir, params$tax_level, cohort_subdir, "microbiome_panel_MainText.svg"), width = 11, height = 13)
...
ggsave(plot = microbiome_panel_Supplementary, filename = file.path(params$basic_outdir, params$tax_level, cohort_subdir, "microbiome_panel_Supplementary.svg"), width = 10, height = 4)
...
### Mediation analysis on `beam_slips_n` {.tabset}
...
as.data.frame(reducedDim(MetaPhlAn_Fp7.tse, dimRedName))) %>% 
  ...
  filter(
    ...
    complete.cases(dplyr::select(., contains("PCo"), genotype_treatment, beam_slips_n)), 
...
table(mediationData.df$genotype_treatment)
...
mediations_micrombiome <- sapply(grepv("PCo", colnames(mediationData.df)), function(x) {
  ...
  outcome.lm <- lm(as.formula(paste("beam_slips_n ~ treatment",  x, sep = "+")), data = mediationData.df)
  return(
    list(
...
mediations_micrombiome %>% 
  lapply("[[", "mediation.analysis") %>% 
  lapply(tidy_mediation) %>% 
  bind_rows(.id = "PCo") %>% 
write_tsv(file = file.path(params$basic_outdir, params$tax_level, cohort_subdir, "PCo1-3_mediation_beam_slips_n_ASO_treat_vs_ASO_ctrl.tsv"))
```
...
## Constipation association with the microbiome
...
mediations_micrombiome %>% 
  ...
  lapply(tidy_mediation) %>% 
  bind_rows(.id = "PCo") %>% 
write_tsv(file = file.path(params$basic_outdir, params$tax_level, cohort_subdir, "PCo1-3_mediation_bead_expulsion_min_ASO_treat_vs_ASO_ctrl.tsv"))
```
...
bCL_colData_for_PCoA.df <- cbind.data.frame(as.data.frame(colData(MetaPhlAn_bCL.tse)), reducedDim(MetaPhlAn_bCL.tse, dimRedName) %>% as.data.frame() %>% set_names(paste("MDS", 1:ncol(.))))

beta_plot_basic <- bCL_colData_for_PCoA.df %>% 
ggplot(aes(x = `MDS 1`, y = `MDS 2`, color = genotype_treatment_legend, fill = genotype_treatment_legend, label = cage)) + 
  geom_text() +
  ggConvexHull::geom_convexhull(show.legend = FALSE, alpha = 0.2) + 
  scale_color_manual(values = joe_palette) +
...
bCL_colData_for_PCoA.df <- cbind.data.frame(as.data.frame(colData(MetaPhlAn_bCL_noWT.tse)), reducedDim(MetaPhlAn_bCL_noWT.tse, dimRedName) %>% as.data.frame() %>% set_names(paste("MDS", 1:ncol(.))))
...
panelB_PCoA <- bCL_colData_for_PCoA.df %>% 
ggplot(aes(x = `MDS 1`, y = `MDS 2`, color = genotype_treatment_legend, fill = genotype_treatment_legend)) + 
  geom_point(size = 2.5, stroke = 1, color = "black") +
  geom_point(size = 2.5) +
  ggConvexHull::geom_convexhull(show.legend = FALSE, alpha = 0.2) + 
...
mediationData.df <- cbind.data.frame(
  as.data.frame(colData(MetaPhlAn_bCL.tse)), 
as.data.frame(reducedDim(MetaPhlAn_bCL.tse, dimRedName))) %>% 
  dplyr::rename(
    PCo.1 = V1, 
    PCo.2 = V2,
    PCo.3 = V3
...
```{r}
write_tsv(filter(GTDB.df, SGB %in% gsub("t__", "", rowData(MetaPhlAn.tse)$SGB)), file = file.path(params$basic_outdir, params$tax_level, "Supplementary table 2 - taxonomy GTDB.tsv"))
```
