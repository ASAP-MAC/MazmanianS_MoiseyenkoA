...
# rename SGBs into `species SGB` only if sgb is not already there
SGB_names <- gsub("t__", "", rownames(MetaPhlAn.tse))
vgrepl <- Vectorize(grepl)

rowData(MetaPhlAn.tse)$SGB <- ifelse(vgrepl(SGB_names, rowData(MetaPhlAn.tse)$species), rowData(MetaPhlAn.tse)$species, paste(rowData(MetaPhlAn.tse)$species, SGB_names))
...
# add taxonomy
genotype_differences_in_ctrls.maaslin3_curated <- add_taxonomy_to_maaslin3(maaslin3_res = genotype_differences_in_ctrls.maaslin3, tmp.tse, taxLevel = params$tax_level)
# write tables
...
# add taxonomy
treatments_vs_Control_in_Thy1ASO.maaslin3_curated <- add_taxonomy_to_maaslin3(maaslin3_res = treatments_vs_Control_in_Thy1ASO.maaslin3, tmp.tse, taxLevel = params$tax_level)
# write tables
...
panelD_Obs_boxplot <- alpha_data.df %>% 
  ...
  scale_color_manual(values = joe_palette) +
  scale_fill_manual(values = joe_palette) +
  stat_pvalue_manual(stats_obs, label = "p.adj.round3", tip.length = 0.02) + 
...
panelE_Shannon_boxplot <- alpha_data.df %>% 
  ...
  scale_color_manual(values = joe_palette) +
  scale_fill_manual(values = joe_palette) +
  stat_pvalue_manual(stats_shannon, label = "p.adj.round3", tip.length = 0.02) + 
  theme(axis.text.x = element_blank(), legend.text = element_markdown()) +
  labs(
    x = ""
  )
...
panelF_FB.Ratio <- panelF_FB.Ratio.df %>% 
  ...
  scale_color_manual(values = joe_palette) +
  scale_fill_manual(values = joe_palette) +
  stat_pvalue_manual(stats_fbrartio, label = "p.adj.round3", tip.length = 0.02) + 
  theme(axis.text.x = element_blank(), legend.text = element_markdown()) +
  labs(
    x = "",
...
panelF_FB.Ratio_horiz <- panelF_FB.Ratio.df %>% 
  ...
  scale_fill_manual(values = joe_palette) +
  coord_flip() + 
  stat_pvalue_manual(stats_fbrartio, label = "p.adj.round3", tip.length = 0.02, coord.flip = TRUE) + 
...
beam_steps_assoc_microbiome_clean <- biobakeryUtils::add_taxonomy_to_maaslin3(beam_steps_assoc_microbiome, MetaPhlAn_Fp7.tse, taxLevel = params$tax_level)
...
pairwise_dissim_bray <- reshape2::melt(mtx) %>% 
  filter(complete.cases(.)) %>%
  dplyr::inner_join(., as.data.frame(colData(MetaPhlAn_bCL.tse)) %>% transmute(Var1 = uuid, genotype_treatment_legend = genotype_treatment_legend), by = "Var1") %>% 
  dplyr::inner_join(., as.data.frame(colData(MetaPhlAn_bCL.tse)) %>% transmute(Var2 = uuid, genotype_treatment_legend = genotype_treatment_legend), by = "Var2") %>% 
  select(Var1, Var2, contains(c("uuid", "genotype", "treatment")), value) %>% 
  mutate(pairs = paste(genotype_treatment_legend.y, genotype_treatment_legend.x, sep = " vs ")) %>% 
  filter(genotype_treatment_legend.x != genotype_treatment_legend.y)
...
pairwise_adonis_stats_asterisks <- pairwise_adonis_stats %>% 
  mutate(pairs = gsub("Thy1-ASO benCom", "Thy1-ASO benCom", pairs))
...
stats_obs <- rstatix::pairwise_wilcox_test(alpha_data.df, formula = `Observed Richness` ~ genotype_treatment,p.adjust.method =  "BH") %>% 
  mutate(group2 = gsub("benCom", "*benCom*", group2),
         p.adj.round3 = round(p.adj, 3)) %>% 
  rstatix::add_y_position()
...
comparisons <- combn(levels(colData(MetaPhlAn_bCL.tse)$genotype_treatment_legend), 2, simplify = FALSE)
...
beam_steps_assoc_microbiome_clean <- biobakeryUtils::add_taxonomy_to_maaslin3(beam_steps_assoc_microbiome, MetaPhlAn_bCL.tse, taxLevel = params$tax_level)
...
# add taxonomy
cohort_differences.maaslin3_curated <- add_taxonomy_to_maaslin3(maaslin3_res = cohort_differences.maaslin3, tmp.tse, taxLevel = params$tax_level)
# write tables
