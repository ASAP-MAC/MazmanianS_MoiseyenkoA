...
joe_palette <- c("WT Control" = "#b2df8a", "Thy1-ASO Control" = "#a6cee3", "Thy1-ASO Fp" = "#1f78b4", "Thy1-ASO benCom" = "turquoise4")
```
...
metadata.df <- read_tsv("Data/metadata_5.0.tsv") %>% 
  ...
  mutate(
    ...
    genotype_treatment = paste(genotype, treatment) %>% factor(levels = c("WT Control", "Thy1-ASO Control", "Thy1-ASO Fp", "Thy1-ASO benCom"))
    )
...
# add taxonomy
treatments_vs_Control_in_Thy1ASO.maaslin3_curated <- add_taxonomy_to_maaslin3(maaslin3_res = treatments_vs_Control_in_Thy1ASO.maaslin3, tmp.tse)
# write tables
...
microbiome_panel_Supplementary <- ggarrange(
  ggarrange(
    panelD_Obs_boxplot + theme(legend.position = "none"),
    panelE_Shannon_boxplot + xlab(" ") + theme(axis.title.x = element_text(size = 34), legend.position = "none"),
    nrow = 2, labels = c("A", "B")),
  panelG_diffAbund, 
  widths = c(0.5,1), 
  labels = c("", "C")
...
tmp.tse <- MetaPhlAn_bCL.tse[,MetaPhlAn_bCL.tse@colData$treatment == "Control"] 
tmp.tse@colData$genotype <- relevel(tmp.tse@colData$genotype, "WT")
tmp.tse@colData$genotype <- droplevels(tmp.tse@colData$genotype)
...
panelG_diffAbund <- readRDS("results/SGB/cohort_bCL/treatments_vs_Control_in_Thy1-ASO/summary_plot_gg.RDS") + 
  theme_light() + 
  theme(strip.text = element_blank(), axis.text.y = element_text(size = 12), axis.title.x = element_text(size = 12), axis.title.y = element_blank()) 
