...
panelC_pairwise_dissim_boxplot_horiz <- pairwise_dissim_bray %>%
  ...
  # Apply the colorize_labels function to your x-axis labels
  scale_y_discrete(labels = function(x) colorize_labels(x, color_map)) +
  # Tell ggplot to render the axis text as Markdown/HTML
...
set.seed(1234)
library(vegan)
pairwise_adonis_stats <- pairwise.adonis(x = as.dist(metadata(MetaPhlAn_bCL.tse)[[dissimName]]), 
...
color_map <- set_names(ggsci::pal_jco()(5)[c(3, 2, 1)]) %>% set_names(c("WT\\ Vehicle", "ASO\\ Vehicle", "ASO\\ benCom"))
