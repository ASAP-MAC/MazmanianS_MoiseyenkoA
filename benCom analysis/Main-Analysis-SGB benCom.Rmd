...
overall_color_palette <- c("WT Control" = "#b2df8a", "Thy1-ASO Control" = "#a6cee3", "Thy1-ASO benCom" = "darkslategray")
...
# print input data for the benCom cohort
MetaPhlAn.tse <- MetaPhlAn.tse[,MetaPhlAn.tse$cohort == "bCL"]
MetaPhlAn.tse
```
...
```{r}
set.seed(1234)
dimRedName <- paste("benCom", params$beta_dist, "mds", sep = "_")
...
benCom_colData_for_PCoA.df <- cbind.data.frame(as.data.frame(colData(MetaPhlAn.tse)), reducedDim(MetaPhlAn.tse, dimRedName) %>% as.data.frame() %>% set_names(paste("MDS", 1:ncol(.))))

panelB_PCoA <- benCom_colData_for_PCoA.df %>% 
...
```{r}
dissimName <- paste("benCom", params$beta_dist, sep = "_")
  ...
  scale_y_discrete(
    labels = function(y) {
      ...
      altnamesItalics <- strsplit(altnamesBasic, "\\ ") %>% lapply(function(x) 
        ifelse(str_ends(x, "cter|culum|terium|cter|monas|ccus|llus|cus|ctor|ella|ciens|spira"), paste0("*", x, "*"), x)) %>% 
...
motor_gi_symptoms.list <- sapply(c("Beam_steps", "Bead_exp"), function(x) openxlsx::read.xlsx("Data/benCom_GI_motor_assessment.xlsx", sheet = x), simplify = FALSE, USE.NAMES = TRUE)
...
sapply(c("png", "svg", "pdf"), function(fmt)
ggsave(plot = microbiome_panel_Supplementary, filename = file.path(params$basic_outdir, params$microbiome_data_type, sprintf("Supplementary Figure %i - alpha diversities and FB Ratio.%s", SuppFigIndex, fmt)), width = 10, height = 4)
)
SuppFigIndex <- sum(SuppFigIndex, 1)
...
# Supplementary Figure `r SuppFigIndex` - Distribution of motor and GI symptoms

```{r}
is_boxplot_outlier <- function(x) {
  return(x < quantile(x, 0.25) - 1.5 * IQR(x) | x > quantile(x, 0.75) + 1.5 * IQR(x))
}

pole_descent_boxplot <- colData(MetaPhlAn.tse) %>% 
  as.data.frame() %>% 
  filter(!is.na(slips_over_steps)) %>% 
  group_by(genotype_treatment) %>% 
  mutate(outlier = ifelse(is_boxplot_outlier(slips_over_steps), mouse_id, NA)) %>% 
  ungroup() %>% 
  ggplot(aes(x = genotype_treatment, y = slips_over_steps, color = genotype_treatment)) + 
  geom_boxplot(show.legend = FALSE) + 
  ggbeeswarm::geom_beeswarm(cex = 2, dodge.width = 0.5) +
  ggrepel::geom_label_repel(aes(label = outlier), hjust = - 0.1, show.legend = FALSE) +
  scale_color_manual(values = overall_color_palette, aesthetics = "color") +
  theme(axis.text.x = element_blank()) +
  labs(
    x = NULL,
    y = "Time to descend pole (s)",
    color = "Genotype and Age"
  )

bead_expulsion_boxplot <- colData(MetaPhlAn.tse) %>% 
  as.data.frame() %>% 
  filter(!is.na(bead_expulsion_min)) %>% 
  group_by(genotype_treatment) %>% 
  mutate(outlier = ifelse(is_boxplot_outlier(bead_expulsion_min), mouse_id, NA)) %>% 
  ungroup() %>% 
  ggplot(aes(x = genotype_treatment, y = bead_expulsion_min, color = genotype_treatment)) + 
  geom_boxplot(show.legend = FALSE) + 
  ggbeeswarm::geom_beeswarm(cex = 2, dodge.width = 0.5) +
  ggrepel::geom_label_repel(aes(label = outlier), hjust = - 0.1, show.legend = FALSE) +
  scale_color_manual(values = overall_color_palette, aesthetics = "color") +
...
motor_gi_plots <- ggarrange(pole_descent_boxplot, bead_expulsion_boxplot, common.legend = TRUE)

sapply(c("png", "svg", "pdf"), function(fmt)
ggsave(plot = motor_gi_plots, filename = sprintf(
      "%s/%s/Supplementary Figure %i - Motor and GI performance plots vs genotype and age.%s",
      params$basic_outdir,
      params$microbiome_data_type,
      SuppFigIndex,
      fmt
