...
fermentative_pathways <- sapply(strsplit(fermentative_pathways, ":"), "[", 1)
...
```{r}
assay.df <- assay(humann_pathways_strat) %>% as.data.frame() %>% rownames_to_column("pathway")

per_pathway_contributions <- lapply(fermentative_pathways, function(pwy) filter(assay.df, grepl(pwy, assay.df$pathway)))

per_pathway_contributions %>% 
  lapply(mutate, pathway = sapply(strsplit(pathway, split = "\\|"), "[", 2)) %>% 
  lapply(column_to_rownames, "pathway") %>% 
  lapply(rowSums) %>% 
